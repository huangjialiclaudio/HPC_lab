...
int main(int argc, char* argv[]){
    ...
    // task1 Compute Matrix product and Euclidean norm
    MatrixXd originalImageTrans = originalImage.transpose();
    MatrixXd matrixProduct = originalImageTrans * originalImage; 
    cout<<"Task1: Matrix product norm = "<<to_string(matrixProduct.norm())<<endl;

    // task2 Report the two largest computed singular values of A
    Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> es(matrixProduct);
    if(es.info() != Eigen::Success) abort();
    const Eigen::VectorXd eigenvalues = es.eigenvalues().real();
    cout << "Task2: The two largest eigenvalues are: " + std::to_string(eigenvalues(eigenvalues.size() - 1)) + " and " + std::to_string(eigenvalues(eigenvalues.size() - 2)) << endl;

    // task3 Export matrix
    saveMarket(matrixProduct,"../lis-2.1.6/test/task3.mtx");



    // create image
    MatrixXd image(200,200);
    for(int i = 0; i < 200; i++){
        for(int j = 0; j < 200; j++){
            if((i / 25) % 2 != ((j / 25) % 2 )){
                image(i,j) = 255;
    ...
    Eigen::Matrix<unsigned char, Dynamic, Dynamic, RowMajor> image_result(200, 200);

    image_result = image.unaryExpr([](double val) -> unsigned char {
        return static_cast<unsigned char>(val);
    });
    // Save the grayscale image using stb_image_write
    const std::string output_image_path = "image.png";
    if (stbi_write_png(output_image_path.c_str(), 200, 200, 1,
                        image_result.data(), 200) == 0) {
